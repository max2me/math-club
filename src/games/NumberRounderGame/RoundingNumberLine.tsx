...
export function RoundingNumberLine({ number, divisor, isWin, showMidpoint = true, showTarget = true, showRoundButtons = true }: RoundingNumberLineProps) {
  ...
  const midFraction = ((lowerBound + upperBound) / 2 - rangeBottom) / totalRange;
        ...
        {containerHeight > 0 && (
          <>
            ...
            {ticks.map((tick) => {
              ...
              const isBound = tick.value === lowerBound || tick.value === upperBound;
              ...
              return (
                <div
                  ...
                  style={{ top: fractionToTop(tick.fraction), transform: 'translateY(-50%)', right: 4, opacity: hidden ? 0 : 1 }}
                >
                  <span className={`flex-1 text-right text-[10px] sm:text-xs font-bold tabular-nums whitespace-nowrap ${tick.isMidpoint ? 'text-slate-500' : isAnswer ? 'text-emerald-400' : isBound ? 'text-slate-200' : 'text-slate-500'}`}>
                    {format(tick.value)}
                  </span>
                  <div className={`w-7 h-px ml-1 ${tick.isMidpoint ? 'bg-slate-500' : isBound ? 'bg-slate-300' : 'bg-slate-600'}`} />
                </div>
              );
            })}

            {/* Target number marker on the right */}
            <div
            ...
            >
              ...
              <span className="ml-1.5 flex">
                {format(number).split('').map((char, i) => (
                  <span
                    key={i}
                    className={`text-[10px] sm:text-xs font-bold tabular-nums ${char === ',' ? 'text-purple-300/50' : 'text-purple-300'}`}
